...
# kubectl port-forward service/lidarr -n default 8989:80
...
if [[ "${lidarr_eventtype:-}" == "Test" ]]; then
    ...
    PUSHOVER_MESSAGE="Howdy this is a test notification from ${lidarr_instancename:-Lidarr}"
fi
...
if [[ "${lidarr_eventtype:-}" == "Download" ]]; then
    if [[ "${lidarr_isupgrade}" == "True" ]]; then pushover_title="Upgraded"; else pushover_title="Downloaded"; fi
    printf -v PUSHOVER_TITLE "Episode %s" "${pushover_title}"
    printf -v PUSHOVER_MESSAGE "<b>%s (S%02dE%02d)</b><small>\n%s</small><small>\n\n<b>Client:</b> %s</small><small>\n<b>Quality:</b> %s</small>" \
        "${lidarr_series_title}" \
        "${lidarr_episodefile_seasonnumber}" \
        "${lidarr_episodefile_episodenumbers}" \
        "${lidarr_episodefile_episodetitles}" \
        "${lidarr_download_client}" \
        "${lidarr_episodefile_quality}"
    printf -v PUSHOVER_URL "%s/series/%s" "${lidarr_applicationurl:-localhost}" "${lidarr_series_titleslug}"
    printf -v PUSHOVER_URL_TITLE "View series in %s" "${lidarr_instancename:-Lidarr}"
fi

#
# Send notification on Manual Interaction Required
#
if [[ "${lidarr_eventtype:-}" == "ManualInteractionRequired" ]]; then
    PUSHOVER_PRIORITY="1"
    printf -v PUSHOVER_TITLE "Episode requires manual interaction"
    printf -v PUSHOVER_MESSAGE "<b>%s</b><small>\n<b>Client:</b> %s</small>" \
        "${lidarr_series_title}" \
        "${lidarr_download_client}"
    printf -v PUSHOVER_URL "%s/activity/queue" "${lidarr_applicationurl:-localhost}"
    printf -v PUSHOVER_URL_TITLE "View queue in %s" "${lidarr_instancename:-Lidarr}"
fi
